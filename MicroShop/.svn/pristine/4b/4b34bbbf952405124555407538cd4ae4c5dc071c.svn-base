...
- (BOOL) createTestList:(sqlite3*)db {
    ...
    //这句是大家熟悉的SQL语句
    char *sql = "create table if not exists cache(ID INTEGER PRIMARY KEY AUTOINCREMENT, testID int,testValue text,testName text)";// testID是列名，int 是数据类型，testValue是列名，text是数据类型，是字符串类型
    ...
    NSLog(@"Create table 'cache' successed.");
    return YES;
}

//插入数据
-(BOOL) insertTestList:(SqlDataList *)insertList {
    ...
    if ([self openDB]) {
        ...
        static char *sql = "INSERT INTO cache(testID, testValue,testName) VALUES(?, ?, ?)";
        
        int success2 = sqlite3_prepare_v2(_database, sql, -1, &statement, NULL);
        if (success2 != SQLITE_OK) {
            NSLog(@"Error: failed to insert:cache");
...
- (NSMutableArray*)getTestList{
    ...
    if ([self openDB]) {
        ...
        //sql语句
        char *sql = "SELECT testID, testValue ,testName FROM cache";//从cache这个表中获取 testID, testValue ,testName，若获取全部的话可以用*代替testID, testValue ,testName。
        ...
        if (sqlite3_prepare_v2(_database, sql, -1, &statement, NULL) != SQLITE_OK) {
            ...
            while (sqlite3_step(statement) == SQLITE_ROW) {
                SqlDataList* sqlList = [[SqlDataList alloc] init] ;
...
//更新数据
-(BOOL) updateTestList:(SqlDataList *)updateList{
    
    if ([self openDB]) {
        sqlite3_stmt *statement;//这相当一个容器，放转化OK的sql语句
        //组织SQL语句
        char *sql = "update cache set testValue = ? and testName = ? WHERE testID = ?";
        
        //将SQL语句放入sqlite3_stmt中
        int success = sqlite3_prepare_v2(_database, sql, -1, &statement, NULL);
        if (success != SQLITE_OK) {
            NSLog(@"Error: failed to update:cache");
            sqlite3_close(_database);
            return NO;
        }
...
//删除数据
- (BOOL) deleteTestList:(SqlDataList *)deletList{
    if ([self openDB]) {
        
        sqlite3_stmt *statement;
        //组织SQL语句
        static char *sql = "delete from cache where testID = ? and testValue = ? and testName = ?";
        //将SQL语句放入sqlite3_stmt中
        int success = sqlite3_prepare_v2(_database, sql, -1, &statement, NULL);
        if (success != SQLITE_OK) {
            NSLog(@"Error: failed to delete:cache1");
            sqlite3_close(_database);
            return NO;
        }
...
- (NSMutableArray*)searchTestList:(NSString*)searchString{
    ...
    if ([self openDB]) {
        ...
        sqlite3_stmt *statement = nil;
        //sql语句
        NSString *querySQL = [NSString stringWithFormat:@"SELECT * from cache where testName like \"%@\"",searchString];
        const char *sql = [querySQL UTF8String];
        //        char *sql = "SELECT * FROM cache WHERE testName like ?";//这里用like代替=可以执行模糊查找，原来是"SELECT * FROM cache WHERE testName = ?"
        if (sqlite3_prepare_v2(_database, sql, -1, &statement, NULL) != SQLITE_OK) {
            NSLog(@"Error: failed to prepare statement with message:search testValue.");
            return NO;
        } else {
            //SqlDataList *searchList = [[SqlDataList alloc]init];
